...
require "BayesianSet_no_cleaning.rb"
require "rubygems"
require "twitter"
...
def find_focus_words(tweet, set)
	clean_tweet = tweet.dup #clean_text(tweet.dup)
	words = []
	probs = []
	focus_words = []
	
	clean_tweet.downcase.split(" ").map {|word| words << word}
  ...
  for i in 0...@num_trends
      ...
      tweets << t.text
		...
		for j in 0..tweets.length - 1
			...
			p = calc_probability_of_n(tweets[j], set)
	...
	for i in 0..tweets.length - 1
		p = calc_probability_of_n(tweets[i], set)
